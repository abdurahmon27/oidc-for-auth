# ADR 0001: Telegram login via bot-delivered OTP

- Status: Accepted
- Date: 2026-07-01
- Applies to: `server-driven/backend` + `server-driven/frontend` and the `frontend-driven/` module (see its own copy of this ADR). Both modules implement the same flow.
...
API access token. In practice the configured `TELEGRAM_API_TOKEN` was an
ordinary **BotFather bot token**, so every send failed with
`ACCESS_TOKEN_INVALID` (surfaced to the client as a misleading HTTP 429).
...
3. The web app shows the bot username and the deep link. The user opens it and
...
5. The user clicks **"Enter OTP"** in the web app (revealing the input),
   copies the code from Telegram, and submits it.
